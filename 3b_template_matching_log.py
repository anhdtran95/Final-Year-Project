...
for index, f in enumerate(X_train):
    ...
    mypic_rev_gauss = sp.ndimage.gaussian_filter(
        mypic_rev_log, sigma=3)  # Gaussian filter
    Segment_Row = []
    for s in range(len(LOG_SPEC_SEGMENTS)):
        ...
        if(s%500==0):
            print("matching template for: " + str(s) + " out of " + str(len(LOG_SPEC_SEGMENTS)))

    TRAIN_LOG_SPEC_FEATURES.append(Segment_Row)


TRAIN_LOG_SPEC_FEATURES = np.array(TRAIN_LOG_SPEC_FEATURES)

# SAVE THE FEATURES for the TRAINING SET
output = open(template_folder + 'TRAIN_LOG_SPEC_FEATURES_freq5.pkl', 'wb')
pickle.dump(TRAIN_LOG_SPEC_FEATURES, output)
output.close()

TRAIN_LOG_SPEC_LABELS = np.array(y_train)

# SAVE THE LABELS for the TRAINING SET
output = open(template_folder + 'TRAIN_LOG_SPEC_LABELS_freq5.pkl', 'wb')
pickle.dump(TRAIN_LOG_SPEC_LABELS, output)
output.close()
...
for index, f in enumerate(X_test):
    ...
    TEST_LOG_SPEC_FEATURES.append(Segment_Row)


TEST_LOG_SPEC_FEATURES = np.array(TEST_LOG_SPEC_FEATURES)

# SAVE THE FEATURES for the TEST SET
output = open(template_folder + 'TEST_LOG_SPEC_FEATURES_freq5.pkl', 'wb')
pickle.dump(TEST_LOG_SPEC_FEATURES, output)
output.close()


TEST_LOG_SPEC_LABELS = np.array(y_test)

# SAVE THE LABELS for the TESTING SET
output = open(template_folder + 'TEST_LOG_SPEC_LABELS_freq5.pkl', 'wb')
pickle.dump(TEST_LOG_SPEC_LABELS, output)
output.close()
